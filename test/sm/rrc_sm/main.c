...
#include "../../../src/sm/rrc_sm/rrc_sm_agent.h"
#include "../../../src/sm/rrc_sm/rrc_sm_ric.h"
...
{
  assert(read != NULL);
  assert(read->type == RRC_STATS_V0);

  fill_rrc_ind_data(&read->rrc_stats);
  cp.hdr = cp_rrc_ind_hdr(&read->rrc_stats.hdr);
  cp.msg = cp_rrc_ind_msg(&read->rrc_stats.msg);
}
...
void check_indication(sm_agent_t* ag, sm_ric_t* ric)
{
  ...
  assert(msg.type == RRC_STATS_V0);
  rrc_ind_data_t* data = &msg.rrc_stats;

  assert(eq_rrc_ind_hdr(&data->hdr, &cp.hdr) == true);
  assert(eq_rrc_ind_msg(&data->msg, &cp.msg) == true);
  assert(eq_rrc_call_proc_id(data->proc_id, cp.proc_id) == true);

  free_rrc_ind_hdr(&data->hdr);
  free_rrc_ind_msg(&data->msg);
...
int main()
{
  ...
  sm_agent_t* sm_ag = make_rrc_sm_agent(io_ag);
  sm_ric_t* sm_ric = make_rrc_sm_ric();
