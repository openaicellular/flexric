...
byte_array_t ccc_enc_event_trigger_json(e2sm_ccc_event_trigger_t const* event_trigger)
{
  ...
  cJSON *event_trigger_json = NULL;
  cJSON *event_trigger_format_json = NULL;

  cJSON *object = cJSON_CreateObject();
  assert(object != NULL);

  event_trigger_json = cJSON_CreateString("RICEventTriggerDefinition");

  cJSON_AddItemToObject(object, "name", event_trigger_json);
  ...
  cJSON_AddItemToObject(object, "eventTriggerDefinitionFormat", event_trigger_format_json);
