...
byte_array_t kpm_enc_action_def_asn(kpm_action_def_t const *action_def)
{
  ...
  case E2SM_KPM_ActionDefinition__actionDefinition_formats_PR_actionDefinition_Format1:
    ...
    int ret;
    assert((action_def->MeasInfo_len <= maxnoofMeasurementInfo && action_def->MeasInfo_len > 0) && "Number of measures not allowed");
    for (size_t i = 0; i < action_def->MeasInfo_len; i++)
    {
      ...
      assert((action_def->MeasInfo[i].labelInfo_len <= maxnoofLabelInfo && action_def->MeasInfo[i].labelInfo_len > 0) && "Number of labels not allowed");
    ...
    // pdu->actionDefinition_formats.choice.actionDefinition_Format2 = calloc (1, sizeof(E2SM_KPM_ActionDefinition_Format2_t));
...
byte_array_t kpm_enc_ind_msg_asn(kpm_ind_msg_t const *ind_msg)
{
  ...
  // 1. measData
  assert((ind_msg->MeasData_len <= maxnoofMeasurementRecord && ind_msg->MeasData_len > 0) && "Number of records not allowed");
  for (size_t i = 0; i < ind_msg->MeasData_len; i++)
  {
    ...
    assert(ind_msg->MeasData[i].measRecord_len <= maxnoofMeasurementValue && ind_msg->MeasData[i].measRecord_len > 0 && "Number of Values not allowed");
...
static LabelInfoItem_t *cp_label_info_item_into_asn(adapter_LabelInfoItem_t const *src)
{
  ...
  if (src->sliceID != NULL)
  {
    dst->measLabel.sliceID = malloc(sizeof(*(dst->measLabel.sliceID)));
    assert(dst->measLabel.sliceID != NULL && "Memory exhausted");
    *(dst->measLabel.sliceID) = *(src->sliceID);
  }

  if (src->fiveQI != NULL)
  {
    dst->measLabel.fiveQI = malloc(sizeof(*(dst->measLabel.fiveQI)));
    assert(dst->measLabel.fiveQI != NULL && "Memory exhausted");
    *(dst->measLabel.fiveQI) = *(src->fiveQI);
  }

  if (src->qFI != NULL)
  {
    dst->measLabel.qFI = malloc(sizeof(*(dst->measLabel.qFI)));
    assert(dst->measLabel.qFI != NULL && "Memory exhausted");
    *(dst->measLabel.qFI) = *(src->qFI);
  }
  
  if (src->qCI != NULL)
  {
   dst->measLabel.qCI = malloc(sizeof(*(dst->measLabel.qCI)));
    assert(dst->measLabel.qCI != NULL && "Memory exhausted");
    *(dst->measLabel.qCI) = *(src->qCI);
  }

  if (src->qCImax != NULL)
  {
    dst->measLabel.qCImax = malloc(sizeof(*(dst->measLabel.qCImax)));
    assert(dst->measLabel.qCImax != NULL && "Memory exhausted");
    *(dst->measLabel.qCImax) = *(src->qCImax);
  }

  if (src->qCImin != NULL)
  {
    dst->measLabel.qCImin = malloc(sizeof(*(dst->measLabel.qCImin)));
    assert(dst->measLabel.qCImin != NULL && "Memory exhausted");
    *(dst->measLabel.qCImin) = *(src->qCImin);
  }

  if (src->aRPmax != NULL)
  {
    dst->measLabel.aRPmax= malloc(sizeof(*(dst->measLabel.aRPmax)));
    assert(dst->measLabel.aRPmax != NULL && "Memory exhausted");
    *(dst->measLabel.aRPmax) = *(src->aRPmax);
  }
  
  if (src->aRPmin != NULL)
  {
    dst->measLabel.aRPmin = malloc(sizeof(*(dst->measLabel.aRPmin)));
    assert(dst->measLabel.aRPmin!= NULL && "Memory exhausted");
    *(dst->measLabel.aRPmin) = *(src->aRPmin);
  }

  if (src->bitrateRange != NULL)
  {
    dst->measLabel.bitrateRange = malloc(sizeof(*(dst->measLabel.bitrateRange)));
    assert(dst->measLabel.bitrateRange!= NULL && "Memory exhausted");
    *(dst->measLabel.bitrateRange) = *(src->bitrateRange);
  }

  if (src->layerMU_MIMO != NULL)
  {
    dst->measLabel.layerMU_MIMO = malloc(sizeof(*(dst->measLabel.layerMU_MIMO)));
    assert(dst->measLabel.layerMU_MIMO!= NULL && "Memory exhausted");
    *(dst->measLabel.layerMU_MIMO) = *(src->layerMU_MIMO);
  }

  if (src->sUM!= NULL)
  {
    dst->measLabel.sUM = malloc(sizeof(*(dst->measLabel.sUM)));
    assert(dst->measLabel.sUM!= NULL && "Memory exhausted");
    *(dst->measLabel.sUM) = *(src->sUM);
  }

  if (src->distBinX!= NULL)
  {
    dst->measLabel.distBinX= malloc(sizeof(*(dst->measLabel.distBinX)));
    assert(dst->measLabel.distBinX!= NULL && "Memory exhausted");
    *(dst->measLabel.distBinX) = *(src->distBinX);
  }

  if (src->distBinY!= NULL)
  {
    dst->measLabel.distBinY= malloc(sizeof(*(dst->measLabel.distBinY)));
    assert(dst->measLabel.distBinY!= NULL && "Memory exhausted");
    *(dst->measLabel.distBinY) = *(src->distBinY);
  }

  if (src->distBinZ!= NULL)
  {
    dst->measLabel.distBinZ= malloc(sizeof(*(dst->measLabel.distBinZ)));
    assert(dst->measLabel.distBinZ!= NULL && "Memory exhausted");
    *(dst->measLabel.distBinZ) = *(src->distBinZ);
  }

  if (src->preLabelOverride!= NULL)
  {
    dst->measLabel.preLabelOverride= malloc(sizeof(*(dst->measLabel.preLabelOverride)));
    assert(dst->measLabel.preLabelOverride!= NULL && "Memory exhausted");
    *(dst->measLabel.preLabelOverride) = *(src->preLabelOverride);
  }

  if (src->startEndInd!= NULL)
  {
    dst->measLabel.distBinX= malloc(sizeof(*(dst->measLabel.startEndInd)));
    assert(dst->measLabel.startEndInd!= NULL && "Memory exhausted");
    *(dst->measLabel.startEndInd) = *(src->startEndInd);
  }

  if (src->min!= NULL)
  {
    dst->measLabel.min = malloc(sizeof(*(dst->measLabel.min)));
    assert(dst->measLabel.min != NULL && "Memory exhausted");
    *(dst->measLabel.min) = *(src->min);
  }

  if (src->max!= NULL)
  {
    dst->measLabel.max= malloc(sizeof(*(dst->measLabel.max)));
    assert(dst->measLabel.max!= NULL && "Memory exhausted");
    *(dst->measLabel.max) = *(src->max);
  }

  if (src->avg!= NULL)
  {
    dst->measLabel.avg= malloc(sizeof(*(dst->measLabel.avg)));
    assert(dst->measLabel.avg!= NULL && "Memory exhausted");
    *(dst->measLabel.avg) = *(src->avg);
  }
