...
void free_g_gnb_id_lst(g_gnb_id_lst_t* src)
{
  ...
  for (size_t i = 0; i < src->sz_g_gnb_id_lst; i++)
    free(src->g_gnb_id_lst[i]);

  free(src->g_gnb_id_lst);
}
...
bool eq_g_gnb_id_lst(g_gnb_id_lst_t const* m0, g_gnb_id_lst_t const* m1)
{
  ...
  if (m0->sz_g_gnb_id_lst != m1->sz_g_gnb_id_lst)
    return false;

  for (size_t i = 0; i < m0->sz_g_gnb_id_lst; i++){
    if (strcmp(m0->g_gnb_id_lst[i], m1->g_gnb_id_lst[i]) != 0){
      return false;
    }
  }
