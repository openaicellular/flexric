...
long
aper_get_cwn(asn_per_data_t *pd, long lb, long ub) {
	assert(ub >= lb);
	long range = ub - lb + 1;
	int range_len;
	int value_len;
	...
	ASN_DEBUG("aper get cwn with lb %ld and ub %ld", lb, ub);
	...
	value_len = aper_get_cwn(pd, 1, range_len);
	if (value_len == -1)
		return -1;
	if (value_len > 4) {
		ASN_DEBUG("todo: aper_get_cwn: value_len > 4");
		return -1;
	}
	if (aper_get_align(pd))
		return -1;
...
aper_put_length(asn_per_outp_t *po, ssize_t lb, ssize_t ub, size_t n, int *need_eom) {
	...
	*need_eom = 0;
	...
	/* X.691 2002 10.9.3.3 */
	if (constrained && ub < 65536)
		return aper_put_cwn(po, lb, ub, n + lb) ? -1 : (ssize_t)n;
...
int
aper_put_cwn(asn_per_outp_t *po, long lb, long ub, long number) {
	assert(ub >= lb);
	long range = ub - lb + 1;
	...
	ASN_DEBUG("aper put cwn %ld with lb %ld and ub %ld", number, lb, ub);
	...
	for (value_len = 1; ; value_len++) {
		int bits = 1 << (8 * value_len);
		if (value < bits)
			break;
	}
	if (aper_put_cwn(po, 1, range_len, value_len))
		return -1;
	if (aper_put_align(po))
		return -1;
