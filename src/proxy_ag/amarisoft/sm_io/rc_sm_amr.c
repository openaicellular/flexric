...
static
guami_t fill_guami(config_get_amr_t const* cfg, ue_lst_amr_t const* ue_amr)
{
  assert(cfg != NULL);
  assert(ue_amr != NULL);
...
static
ue_id_e2sm_t fill_ue_id(config_get_amr_t const* cfg, ue_lst_amr_t const* ue_amr)
{
  assert(cfg != NULL);
  assert(ue_amr != NULL);
...
static
nr_cgi_t fill_nr_cgi(config_get_amr_t const* cfg)
{
  assert(cfg != NULL);
  nr_cgi_t dst = {0}; 
...
static
cell_global_id_t fill_cell_global_id_t(config_get_amr_t const* cfg, ue_lst_amr_t const* ue_amr) 
{
  assert(cfg != NULL);
  assert(ue_amr != NULL);
...
static
seq_ue_info_t fill_seq_ue_info(config_get_amr_t const* cfg, ue_lst_amr_t const* ue_amr)
{
  assert(cfg != NULL);
  assert(ue_amr != NULL);
  seq_ue_info_t dst = {0}; 
