...
static
void free_func_rc_sm_pend(void* key, void* value)
{
  assert(key != NULL);
  assert(value != NULL);
  (void)key;
...
void init_rc_pend_ds(rc_pend_ds_t* src)
{
  ...
  assoc_init(&src->t, sizeof(int), cmp_int, free_func_rc_sm_pend);
...
rc_pend_msg_t* extract_rc_pend_ds(rc_pend_ds_t* src, int msg_id)
{
  ...
  rc_pend_msg_t* k = assoc_rb_tree_extract(&src->t, &msg_id);
  return k;
}
