...
void read_setup_ran(void* data)
{
  ...
  // TODO: Create a value assigned by CMakeLists.txt (check read_setup_ran.c)
  e2ap_ngran_node_t const n = e2ap_ngran_gNB;
  if(n == e2ap_ngran_gNB){
    dst->len_cca = 1;
  ...
  } else if(n == e2ap_ngran_gNB_DU){
    dst->len_cca = 1;
    dst->cca = calloc(1, sizeof(e2ap_node_component_config_add_t));
    assert(dst->cca != NULL);
    // F1AP
    dst->cca[0] = fill_f1ap_e2ap_node_component_config_add();
  } else if(n == e2ap_ngran_gNB_CUCP){
    dst->len_cca = 3;
