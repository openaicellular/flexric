...
func init() {
	RegisterCallback("MAX_NUM_UES", CallbackMaxNumUes)
}

// CallbackMaxNumUes is an example callback function.
func CallbackMaxNumUes(PolicyConfiguration policy.Configuration) {
	...
	for i := 0; i <= int(sm.E2Nodes.Size()-1); i++ {
		...
		maxNumOfUes := int(PolicyConfiguration.Statement.MaxNumberOfUEs)
