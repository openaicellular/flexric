...
static
void sm_cb_kpm_oran_cell(sm_ag_if_rd_t const* rd)
{
  assert(rd != NULL);
  assert(rd->type == INDICATION_MSG_AGENT_IF_ANS_V0);
  assert(rd->ind.type == KPM_STATS_V3_0); 
  ...
  for(int i=0;i<kpm->msg.frm_1.meas_data_lst_len;i++){
    if(kpm->msg.frm_1.meas_data_lst[i].meas_record_lst->value == INTEGER_MEAS_VALUE)
      printf("Measurement Integer \"%s\" = %d\n", kpm->msg.frm_1.meas_info_lst[i].meas_type.name.buf, kpm->msg.frm_1.meas_data_lst[i].meas_record_lst->int_val);
    else if(kpm->msg.frm_1.meas_data_lst[i].meas_record_lst->value == REAL_MEAS_VALUE)
      printf("Measurement Real \"%s\" = %.2f\n", kpm->msg.frm_1.meas_info_lst[i].meas_type.name.buf, kpm->msg.frm_1.meas_data_lst[i].meas_record_lst->real_val);
  }
  
}

void sm_cb_kpm_oran_slice(sm_ag_if_rd_t const* rd)
{
  assert(rd != NULL);
  assert(rd->type == INDICATION_MSG_AGENT_IF_ANS_V0);
  assert(rd->ind.type == KPM_STATS_V3_0); 
  static indicationCount = 0;
